...
class AdvancedFloatingIdea:
    ...
    def __init__(self, text: str, x: float, y: float, font, settings: VisualSettings):
        ...
        # Like visual effects
        self.like_particles = []
        self.heart_animation = 0.0
    
    def add_like(self):
        """Add a like to this idea with visual effects"""
        self.likes += 1
        self.liked_animation = 1.0
        self.like_glow = 1.0
        self.heart_animation = 1.0
        
        # Create like particles
        for _ in range(8):
            particle = EnhancedParticle(self.final_x, self.final_y, (255, 100, 150), "spark")
            particle.vx = random.uniform(-3, 3)
            particle.vy = random.uniform(-4, -1)
            particle.life = 1.5
            particle.decay = 0.02
            self.like_particles.append(particle)
        
        # Update scale based on likes
        self.base_scale = 1.0 + (self.likes * 0.1)  # Grows 10% per like
    ...
    def update(self, settings: VisualSettings, wind_strength=0, gravity_strength=0):
        ...
        # Update like animations
        if self.liked_animation > 0:
            self.liked_animation -= 0.05
        if self.like_glow > 0:
            self.like_glow -= 0.02
        if self.heart_animation > 0:
            self.heart_animation -= 0.03
        ...
        if self.age < settings.entrance_duration:
            ...
            # Use base scale affected by likes
            entrance_scale = 0.3 + 0.7 * min(1.0, progress * 2)
            self.scale = entrance_scale * self.base_scale
        else:
            self.alpha = 255
            # Apply like-based scaling with bounce animation
            bounce_factor = 1.0 + (self.liked_animation * 0.3)
            self.scale = self.base_scale * bounce_factor
...
def render(self, screen, settings: VisualSettings):
        if self.alpha > 10:
            ...
            # Enhanced glow effect with like glow
            base_glow = settings.glow_intensity
            like_glow_boost = self.like_glow * 0.5
            total_glow = base_glow + like_glow_boost
            
            if total_glow > 0:
                glow_surface = text_surface.copy()
                glow_alpha = int(self.alpha * total_glow)
                glow_surface.set_alpha(glow_alpha)
                
                # Enhanced glow for liked ideas
                glow_radius = 3 if self.likes == 0 else min(8, 3 + self.likes)
                glow_offsets = [(dx, dy) for dx in range(-glow_radius, glow_radius+1) 
                            for dy in range(-glow_radius, glow_radius+1) if dx != 0 or dy != 0]
            ...
            # Like highlight border
            if self.likes > 0:
                border_alpha = int(100 + 155 * max(0, self.like_glow))
                border_color = (255, 100 + self.likes * 20, 150)
                border_thickness = min(5, 2 + self.likes // 3)
                border_rect = text_rect.inflate(10, 10)
                pygame.draw.rect(screen, border_color, border_rect, border_thickness, border_radius=8)
            ...
            if self.heart_animation > 0 and self.likes > 0:
                heart_scale = 1.0 + self.heart_animation * 0.5
                heart_size = int(20 * heart_scale)
                heart_pos = (int(text_rect.right + 10), int(text_rect.centery - heart_size//2))
                
                # Simple heart shape using circles
                heart_color = (255, 100, 150, int(255 * self.heart_animation))
                pygame.draw.circle(screen, heart_color[:3], 
                                (heart_pos[0] - heart_size//4, heart_pos[1]), heart_size//3)
                pygame.draw.circle(screen, heart_color[:3], 
                                (heart_pos[0] + heart_size//4, heart_pos[1]), heart_size//3)
                
                # Like count
                if self.likes > 1:
                    like_font = pygame.font.Font(None, 16)
                    like_text = like_font.render(str(self.likes), True, (255, 255, 255))
                    like_rect = like_text.get_rect(center=(heart_pos[0], heart_pos[1] + heart_size//2 + 10))
                    screen.blit(like_text, like_rect)
        ...
        if event.type == pygame.KEYDOWN and self.active:
            ...
            # Tab completion
                ...
                if self.auto_complete_suggestions:
                    ...
                    self.cursor_pos = len(self.text)
                    self.show_suggestions = False
                return None  # Don't let TAB open dev panel when input is active
...
class ProfessionalDeveloperPanel:
        ...
        if self.active_tab in self.controls:
            for control in self.controls[self.active_tab]:
                if hasattr(control, 'handle_event'):
                    result = control.handle_event(event)
                    # Handle dropdown selections
                    if isinstance(control, DropdownMenu) and result:
                        print(f"🎨 Changed {control.label} to {result}")
    ...
    def get_settings(self) -> VisualSettings:
        ...
        if "Visual" in self.controls:
            ...
            # Get dropdown values
            if len(controls) > 4:
                settings.color_scheme = controls[4].selected
                settings.idea_palette = controls[5].selected
        ...
        if "Colors" in self.controls:
            controls = self.controls["Colors"]
            settings.background_color = controls[0].color
            settings.primary_color = controls[1].color
            settings.secondary_color = controls[2].color
            settings.accent_color = controls[3].color
    ...
    def apply_preset(self, preset_name: str):
        ...
        if "Visual" in self.controls:
            ...
            for control in controls[:4]:  # Only sliders
                control.update_handle_pos()
            
            # Update dropdowns
            if len(controls) > 4:
                controls[4].selected = settings.color_scheme
                controls[5].selected = settings.idea_palette
        ...
        if "Colors" in self.controls:
            controls = self.controls["Colors"]
            controls[0].color = settings.background_color
            controls[1].color = settings.primary_color
            controls[2].color = settings.secondary_color
            controls[3].color = settings.accent_color
            # Update RGB sliders for each color picker
            for i, control in enumerate(controls):
                if hasattr(control, 'r_slider'):
                    control.r_slider.val = control.color[0]
                    control.g_slider.val = control.color[1]
                    control.b_slider.val = control.color[2]
                    control.r_slider.update_handle_pos()
                    control.g_slider.update_handle_pos()
                    control.b_slider.update_handle_pos()
    
    def render(self, screen):
        if not self.visible:
            hint_text = "Press TAB to open Developer Panel | F1 for Help | Click ideas to like them!"
    ...
    def render_help_overlay(self):
        ...
        help_sections = {
            "Basic Controls": [
                ...
                "Click on any idea - Like it and watch it grow!",
            ...
            "Developer Panel": [
                ...
                "Visual tab - Font, effects, color schemes",
                "Colors tab - Custom RGB color controls",
            ...
            "Like System": [
                "Click any floating idea to like it",
                "Ideas grow 10% larger with each like",
                "Liked ideas get enhanced visual effects",
                "Heart animation shows when idea is liked",
                "Popular ideas become more prominent",
                "Create natural idea hierarchy through voting"
            ]
        }
